...
while( k < K )
  ...
  if( pure || n1<=minCount || depth(k)>maxDepth ), 
      % if pure, stop1=stop1+1; end 
      % if n1<=minCount, stop2=stop2+1; end
      % if depth(k)>maxDepth, stop3=stop3+1; end
      k=k+1; continue; 
  end
